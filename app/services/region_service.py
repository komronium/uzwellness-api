...
def _slug(text: str) -> str:
    return slugify(text, fallback="region")
...
class RegionService:
    ...
    async def create(self, payload: RegionCreate) -> Region:
        ...
        slug = await resolve_unique_slug(self.db, Region, _slug(slug_seed))
    ...
    async def update(self, region: Region, payload: RegionUpdate) -> Region:
        ...
        if "slug" in data and data["slug"] is not None:
            data["slug"] = await resolve_unique_slug(
                self.db, Region, _slug(data["slug"]), exclude_id=region.id
            )
        elif "name" in data and "slug" not in data:
            data["slug"] = await resolve_unique_slug(
                self.db, Region, _slug(pick_locale(data["name"])), exclude_id=region.id
            )
