...
> подставляет клиенту: он в **сумах**, целым числом, строкой. Для той же брони
> это `"amount": {"value": "1250000"}`. Значение всегда выводится из тийинной
> суммы делением на 100, поэтому показанное клиенту и списанное совпадают.
> Если сумма брони не кратна суму, она округляется до целого — только для
> отображения; списывается ровно тийинная сумма.
