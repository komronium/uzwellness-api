...
Дополнительно принимаются ключи `orderId`, `account`, `user_id`, `userId`,
`booking_code`, `code` — на случай, если в приложении поле называется иначе.
Значение при этом всегда трактуется как номер бронирования, а не как
идентификатор пользователя. По короткому коду брони из 8 символов
(например `A7K2M9QX`) поиск тоже работает.
